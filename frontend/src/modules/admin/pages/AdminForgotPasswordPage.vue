  ...
  <main class="auth-page recovery-page">
    ...
    <section class="recovery-container">
      <div class="recovery-header">
        ...
        <p>Ingresa el correo de tu cuenta de administrador para recibir un código de recuperación.</p>
      </div>
      ...
      <form class="recovery-form" novalidate @submit.prevent="submitResetPassword">
        ...
        <div class="form-step" :class="{ active: step === 1 }" data-step="1">
          <div class="form-group">
            <label for="recovery-identifier">Correo electrónico</label>
            <input
      ...
      <div class="login-redirect">
        ...
        <RouterLink :to="{ name: 'login' }" class="text-link">Volver al inicio de sesión</RouterLink>
      </div>
    </section>
  </main>
</template>
...
import '../../auth/views/ForgotPasswordView.css'
...
function validateIdentifier() {
  ...
  if (!normalized) {
    errors.identifier = 'Ingresa un correo válido.'
    return ''
  }
...
function stopTimers() {
  ...
  if (resendIntervalId) { clearInterval(resendIntervalId); resendIntervalId = null }
  if (requestIntervalId) { clearInterval(requestIntervalId); requestIntervalId = null }
}
...
async function submitVerifyCode() {
  ...
  try {
    const response = await verifyRecoveryCode({ identifier, code: form.code })
